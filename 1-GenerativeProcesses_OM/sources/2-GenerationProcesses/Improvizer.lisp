...
(defclass* improvizer (pythie)
   (
    ...
    (nexteventifnosolution :initform nil :initarg :nexteventifnosolution :accessor nexteventifnosolution)
    (LengthFactorsFromScen :initform '(1 100) :initarg :start-region :accessor LengthFactorsFromScen)
      ...
      (when (eq (NavigationMode self) 'nothing)
        ...
        (if nextindex 
          ...
          (progn               
            ...
            (if (and (nexteventifnosolution self) (< index (maxetat self)))
                (progn
           ...
           (loop for len being the hash-key of found-scen-prefixes using (hash-value idxs)
                 if (and (reduce-eligible-events self idxs) 
                         (and (>= len (nth 0 (LengthFactorsFromScen self))) 
                              (<= len (nth 1 (LengthFactorsFromScen self))))) 
