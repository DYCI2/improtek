...
(format *om-stream* "loading AudioExample-WithMaxInterface~%")
...
(defmethod set-start-region_cycles ((self improvizer) (scen list) (select-cycles list)) 
  (if (or (not select-cycles) (and ( = (nth 0 select-cycles) 0) ( = (nth 1 select-cycles) 0)))
...
;(setf besttranspomode '(t t t))
 
 maxcont '(9 96 96)   ; continuité Omax en beats
...
;--> Define Numvoice (player in Max patch, "Voices") and evaluate
          (progn
            ...
            (setf memory_tune_label_sequence (MakeLabelsFromList (expand_grid (grid memory_tune)) 'harmlabel))
...
;"yes": t, "no": nil
          ...
          ; SELECT CYCLES ----------> CHANGE POST MARTA !!!!
          ;(set-start-region_cycles Improvizer scenario (nth (1- NUMVOICE) select-cycles))
          (set-start-region_cycles Improvizer memory_tune_label_sequence (nth (1- NUMVOICE) select-cycles)) 
