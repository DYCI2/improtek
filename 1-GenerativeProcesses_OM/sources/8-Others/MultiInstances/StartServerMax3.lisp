...
(if (not *flag-loaded-lib*)
    (let ((file-load-improtek (make-pathname :directory (pathname-directory *load-pathname*) :name "LoadImprotek"))) ; plus besoin de LoadImprotekBIS
      (format *om-stream* "loading ~a~%" file-load-improtek)
      (if (om-standalone-p) (load file-load-improtek) (compile&load file-load-improtek))))


; "Global variables"
(defparameter prtRcv 7433) ; original is 7413 gn
(defparameter prtSnd 7434) ; original is 7414 gn
(defparameter prtSndToAntescofo 7435) ; original is 7657 (originally hardcoded in 7-Examples/...ScenarioTextBis)
